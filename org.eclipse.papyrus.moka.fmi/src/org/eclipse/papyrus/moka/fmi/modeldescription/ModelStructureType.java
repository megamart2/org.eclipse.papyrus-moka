...
public interface ModelStructureType extends EObject {
	 ...
	 * 
	 * @param value
	 *            the new value of the '<em>Outputs</em>' containment reference.
	 * @see #getOutputs()
	 * @generated
	 */
	 ...
	 * <!-- end-user-doc -->
	 * <!-- begin-model-doc -->
	 * Ordered list of all exposed state derivatives (and therefore implicitely associated continuous-time states). Exactly all state derivatives of a ModelExchange FMU must be in this list. A CoSimulation FMU need not expose its state derivatives. If a model
	 * has dynamic state selection, introduce dummy state variables. The dependency definition holds for Continuous-Time and for Event Mode (ModelExchange) and for Communication Points (CoSimulation).
	 * <!-- end-model-doc -->
	 * 
	 ...
	 * 
	 * @param value
	 *            the new value of the '<em>Derivatives</em>' containment reference.
	 * @see #getDerivatives()
	 * @generated
	 */
	 ...
	 * <!-- end-user-doc -->
	 * <!-- begin-model-doc -->
	 * Ordered list of all exposed Unknowns in Initialization Mode. This list consists of all variables with (1) causality = "output" and (initial="approx" or calculated"), (2) causality = "calculatedParameter", and (3) all continuous-time states and all state
	 * derivatives (defined with element Derivatives from ModelStructure)with initial=("approx" or "calculated"). The resulting list is not allowed to have duplicates (e.g. if a state is also an output, it is included only once in the list). The Unknowns in
	 * this list must be ordered according to their ScalarVariable index (e.g. if for two variables A and B the ScalarVariable index of A is less than the index of B, then A must appear before B in InitialUnknowns).
	 * <!-- end-model-doc -->
	 * 
	 ...
	 * 
	 * @param value
	 *            the new value of the '<em>Initial Unknowns</em>' containment reference.
